...
/**
 * This file contains QUI\ERP\Employee\Search
 */
...
class Search extends Singleton
{
    ...
    /**
     * search flag for: search only in employee
     *
     * @var bool
     */
    protected $onlyEmployee = true;
    ...
    protected function getQuery($count = false)
    {
        ...
        if (empty($this->filter) && empty($this->search)) {
            ...
            if ($this->onlyEmployee) {
                try {
                    $employeeGroup = Employees::getInstance()->getEmployeeGroupId();
                    $where         = " WHERE usergroup LIKE '%,{$employeeGroup},%'";
                } catch (QUI\Exception $Exception) {
        ...
        if ($this->onlyEmployee) {
            try {
                $employeeGroup = Employees::getInstance()->getEmployeeGroupId();
                $where[]       = 'usergroup LIKE :employeeGroups';

                $binds['employeeGroups'] = [
                    'value' => '%,'.$employeeGroup.',%',
                    'type'  => \PDO::PARAM_STR
                ];
            } catch (QUI\Exception $Exception) {
    ...
    /**
     * set the flag for searching only in the employee group
     */
    public function searchOnlyInEmployee()
    {
        $this->onlyEmployee = true;
    }
